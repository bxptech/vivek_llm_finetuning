...
LOCATION = "us-central1"              # region where you tuned the model
MODEL_ENDPOINT = "projects/myfinetuning-project/locations/us-central1/models/vivek_finetuning6"

# --- Authenticate using Service Account JSON stored in Streamlit secrets ---
if "SERVICE_ACCOUNT_JSON" not in st.secrets:
    st.error("⚠️ Missing SERVICE_ACCOUNT_JSON in Streamlit secrets.")
    st.stop()

service_account_info = json.loads(st.secrets["SERVICE_ACCOUNT_JSON"])
credentials = service_account.Credentials.from_service_account_info(service_account_info)

# Refresh token to verify credentials
try:
